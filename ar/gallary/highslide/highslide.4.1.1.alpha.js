...
animate: function(el, prop, dur, easing, callback) {
	// $(el).animate(prop, dur || 250, 'swing', callback); return;
	var //rate = 25,
		//from = {},
		//to = {},
		//units = {},
	...
	var opt = { duration: dur, easing: easing, complete: callback, step: step };
...
changeSize : function(up, to, dur) {
	...
	hs.animate( this.wrapper, to.wrapper, {
		duration: dur,
		easing: easing,
		step: exp.isHtml ? function(val, args) {
			if (exp.outline && exp.outlineWhileAnimating && args.prop == (exp.fadeInOut ? 'opacity' : 'top'))
				exp.outline.setPosition(1, 0, 1);
...
crossfade : function (up, to) {
	...
	if (this.outline) var wrapStep =  function(val, args) {
		if (args.prop == 'top') exp.outline.setPosition(1);
	};

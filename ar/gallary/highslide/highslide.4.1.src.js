...
animate: function(el, prop, dur, easing, callback) {
	...
	if (timer.length) {
		//timer.unshift(function(){}); /// dummy step
		timers.push(timer);
	...
	try { /// forum/viewtopic.php?t=1693
		...
		for (var i = 0; i < exp.overlays.length; i++) (function() {
			...
			if (o && o.hideOnMouseOut) {
				...
				hs.animate(o, { opacity: over ? o.opacity : 0 }, null, null, 
...
/*fade : function (el, o, oFinal, dur, fn, i, dir) {
	hs.animate(el, { opacity: oFinal }, dur, null, fn);
	...
	/// fade is finished, return
	if (skip || o < 0 || (dir == 1 && o > oFinal)) { 
		if (fn) fn();
		return;
	}
	
	/// if this element is fading in the other direction, reverse it
...
setPosition : function (exp, pos, vis, dur, easing) {
	if (!pos) pos = {
		x: exp.x.pos,
		y: exp.y.pos,
		w: exp.x.get('wsize'),
	...
	/*var self = this, table = this.table, offset = this.offset;
	if (!dur) dur = 0;
	
	/// defaults to the x and y objects
	pos = pos || {
		x: exp.x.pos,
		y: exp.y.pos,
		w: exp.x.get('wsize'),
	...
	hs.animate(table, {
		left: pos.x - offset,
		top: pos.y - offset,
		width: pos.w + 2 * offset,
		height: pos.h + 2 * offset
	}, {
		duration: dur, 
		easing: easing,
		step: function(val, args) {
			...
			if (prop == 'height' || prop == 'width') {
				...
				// set the table cells size
				var stl = self.td[4].style;
				val -= 4 * offset;
				if (val >= 0) stl[prop] = val +'px';
				
				// hide or show the outline
				if (vis && prop == 'height') 
					table.style.visibility = val > 0 ? 'visible' : 'hidden';
					
				if (prop == 'height') console.log(vis, prop, val, table.style.visibility);
			}				
		}
	});
	
	/// adjust the values for the table cells
	pos.w -= 2 * offset;
	pos.h -= 2 * offset;
		
	/// animate the table cells
	hs.animate (this.td[4], {
		width: pos.w,
		height: pos.h
	}, {
		duration: dur, 
		easing: easing,
		step: (vis ? function(val, args) {
			if (args.prop == 'height') {
				table.style.visibility = val < 2 * offset ? 
					'hidden' : 'visible';
			}
		} : null)
	});
	
	/// ie needs all cells to be set
	if (this.hasAlphaImageLoader) {
		hs.animate(this.td[3], {
			height: pos.h
		}, dur, easing);		
		hs.animate(this.td[5], {
			height: pos.h
	*/
},
...
changeSize : function(up, to, dur) {
	...
	*/
	...
	if (this.isHtml) {
		var mcPos = {
			left: (this.x.pos - this.x.tpos + this.x.cb - this.x.tb) +'px',
			top: (this.y.pos - this.y.tpos + this.y.cb - this.y.tb) +'px'
		};
		if (up) {
			hs.setStyles(this.wrapper, {
				left: (this.x.tpos - this.x.cb + this.x.tb) +'px',
				top: (this.y.tpos - this.y.cb + this.y.tb) +'px'
			});
			hs.setStyles(this.mediumContent, mcPos);
			mcPos = { left: 0, top: 0 };
		}
		
		hs.animate(this.mediumContent, mcPos, dur, this.easing);
	}
	/*<? endif ?>s*/
	...
	if (outline && this.outlineWhileAnimating) {
		var o = outline.offset - hs.outlineStartOffset,
			startPos = {
				x: x.tpos + o, 
				y: y.tpos + o, 
				w: x.t - 2 * o, 
		...
		if (up) outline.setPosition(this, startPos); 
		outline.setPosition(this, up ? null : startPos, 1, dur, this.easing); /// animate to final position
	}*/
	/*<? endif ?>*/
	...
	if (this.outline && this.outlineWhileAnimating) {
		var wrapStep =  function(val, args) {
			if (args.prop == 'height') exp.outline.setPosition(exp, 'wrapper', 1);
		}
	}
	...
	/*<? else : ?>s*/
	hs.animate( this.wrapper, to.wrapper, dur, this.easing);
	/*<? endif ?>s*/
	/// animate the content
	hs.animate( this.content, to.content, dur, this.easing, after);
...
fade : function(up, from, to) {
	...
	if (up) {
		...
		hs.animate(this.wrapper, { opacity: 1 });
	}
	/*<? if ($cfg->outlines) : ?>*/
	if (this.outline) {
			...
			(function() {
				...
				setTimeout(function() {
					exp.outline.setPosition(exp, {
						x: exp.x.pos + o, 
						y: exp.y.pos + o,
						w: exp.x.get('wsize') - 2 * o, 
						h: exp.y.get('wsize') - 2 * o
					}, 1);
				}, t);
			})();
		}
...
crossfade : function (up, to) {
	...
	this.wrapper.appendChild(fadeBox);
	/*from = { /// redefine to expand from previous slide
		...
		xp2: lastX.p2,
		/*< ? if ($cfg->autosize) : ?>s* /
		ximgSize: lastX.imgSize || lastX.size,
		ximgPad: lastX.get('imgPad'),
		yimgSize: lastY.imgSize || lastY.size,
		yimgPad: lastY.get('imgPad'),
		/*< ? endif ?>s* /
		ypos: lastY.pos,
		ysize: lastY.size,
		yp1: lastY.p1,
	...
	if (overlayBox) {
		...
		this.wrapper.appendChild(overlayBox);
			
		//for (var i = 0; i < last.overlays.length; i++) {
		//	var oDiv = hs.$('hsId'+ last.overlays[i]);
			
		//	if (oDiv.reuse === exp.key) {
		//		/*<? if ($cfg->viewport) : ?>s*/
		//		if (oDiv.parentNode == hs.viewport) oDiv.hsKey = exp.key;
		//		else
		//		/*<? endif ?>s*/
		//		exp.overlayBox.appendChild(oDiv);
		//	} else hs.animate(oDiv, { opacity: 0 }); /// fade out
		//}
	}			
	/*<? endif ?>s*/
	...
	function end() {		
		exp.wrapper.style.visibility = exp.content.style.visibility = 'visible';
	...
	/*clip: 'rect('
		+ (y.pos - lastY.pos)+'px, ' /// top
		+ (x.size + x.p1 + x.p2 + x.pos + 2 * lastX.cb - lastX.pos) +'px, ' /// right
		+ (y.size + y.p1 + y.p2 + y.pos + 2 * lastY.cb - lastY.pos) +'px, ' /// bottom
		+ (x.pos - lastX.pos)+'px)' /// left
	*/}, dur);
	...
	if (this.outline) var wrapStep =  function(val, args) {		
		if (args.prop == 'height') exp.outline.setPosition(exp, 'wrapper');
	}
	hs.animate(this.wrapper, to.wrapper, {
		duration: dur, 
		callback: end,
	...
	/*<? else : ?>s*/
	hs.animate(this.wrapper, to.wrapper, dur, null, end);
...
crossfadeStep : function (size) {
	try {
		...
		/*this.last.wrapper.style.clip = 'rect('
			+ (size.ypos - this.last.y.pos)+'px, ' /// top
			+ (size.xsize + size.xp1 + size.xp2 + size.xpos + 2 * this.last.x.cb - this.last.x.pos) +'px, ' /// right
			+ (size.ysize + size.yp1 + size.yp2 + size.ypos + 2 * this.last.y.cb - this.last.y.pos) +'px, ' /// bottom
			+ (size.xpos - this.last.x.pos)+'px)'; /// left
			*/
...
createOverlay : function (o) {
	...
	if (this.gotOverlays) { /// backwards compat: defined after expansion 
		...
		if (!overlay.hideOnMouseOut || this.mouseIsOver) hs.animate(overlay, { opacity: overlay.opacity });
	}
...
positionOverlay : function(overlay) {
	...
	/*<? if ($cfg->viewport) : ?>s*/
	if (overlay.parentNode == hs.viewport) {
	...
	/// horizontal
	if (relToVP && overlay.offsetWidth > overlay.parentNode.offsetWidth)  /// don't spill out
...
showOverlays : function() {
	...
	/*<? if ($cfg->thumbstrip) : ?>s*/
	//if (this.slideshow && this.slideshow.thumbstrip) this.slideshow.thumbstrip.fit();
	/*<? endif ?>s*/
	...
	for (var i = 0; i < this.overlays.length; i++) {
		...
		if (!o.hideOnMouseOut || this.mouseIsOver) {
			...
			hs.animate(o, { opacity: o.opacity });
		}
	}
},
...
hs.Thumbstrip = function(slideshow) {
	...
	function selectThumb (i, scrollBy) {
		...
		var as = dom.getElementsByTagName('a'),
			...
			minTblPos = div[offsetWidth] - table[offsetWidth],
	...
	hs.addEventListener(window, 'resize', function() {
		slideshow.exp.positionOverlay(dom.parentNode); /// should apply to all viewport overlays?
		selectThumb();
	});
...
hs.addEventListener(window, 'resize', function() {
	hs.page = hs.getPageSize();
	/*<? if ($cfg->viewport) : ?>*/
	/*if (hs.ieLt7 && hs.viewport) {
		hs.setStyles(hs.viewport, {
			width: hs.page.width +'px',
			height: hs.page.height +'px'
		});
	}*/
	/*<? endif ?>s*/
});

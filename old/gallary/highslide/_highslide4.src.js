/******************************************************************************
Name:    Highslide JS
Version: 4.0b3 (forked from 3.3.14, updated until 3.3.22)
Config: 
...
init : function () {
	if (!hs.container) {
		...
		for (var x in hs.lang) if (hs[x]) hs.lang[x] = hs[x];
...
setObjContainerSize : function(parent, auto) {
	...
	/*<? if ($cfg->flash) : ?>s*/
	if (this.swfObject) {
		c.style.width = this.swfObject[2] +'px';
		c.style.height = this.swfObject[3] +'px';
	}
	/*<? endif ?>s*/
},
...
writeExtendedContent : function () {
	...
	if (this.objectType == 'iframe') {
		...
		if (swfobject) swfobject.embedSWF(a[0], this.body.id, a[2], a[3], a[4], a[5], a[6], a[7], a[8]);	
	}
...
afterExpand : function() {
	...
	/*<? endif ?>s*/
	/*<? if ($cfg->gallery) : ?>s*/
	if (this.slideshow) this.slideshow.btn.next.getElementsByTagName('a')[0].focus();
	/*<? endif ?>s*/
...
preloadNext : function() {
	...
	if (next.onclick && next.onclick.toString().match(/hs\.expand/)) 
...
getInline : function(types, addOverlay) {
	for (var i = 0; i < types.length; i++) {
		var type = types[i], s; /// two types of caption: caption and heading
		/*<? if ($cfg->events) : ?>s*/
		...
		/*<? if ($cfg->gallery) : ?>s*/
		/// 6. Look for numbers
		if (!this[type] && !s && this.number == type) s = '';
		/*<? endif ?>s*/			
		
		if (!this[type] && typeof s != 'undefined') this[type] = hs.createElement('div', 
				{ className: 'highslide-'+ type, innerHTML: s } );
				
		if (addOverlay && this[type]) {
			var o = {}; /// defaults for heading
			for (var x in this[type+'Options']) o[x] = this[type+'Options'][x];
...
getOverlays : function() {
	...
	/*<? endif ?>s*/
	/*<? if ($cfg->dragging) : ?>s*/
	if (this.heading && hs.dragByHeading) this.heading.className += ' highslide-move';
	/*<? endif ?>s*/

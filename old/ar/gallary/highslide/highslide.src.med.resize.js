...
{	
	...
	if (!exp && el.className == 'highslide-resize' && el.tagName == 'TD') {
		for (var i = 0; i < hs.expanders.length; i++) {			
			if (hs.expanders[i] && hs.expanders[i].outline && hs.expanders[i].outline.table == el.parentNode.parentNode.parentNode)
				exp = hs.expanders[i];
				break;
		}
	}
...
hs.Outline =  function (outlineType, onLoad) {
	...
	this.td = [];
	var resizes = ['nw', 'n', 'ne', 'w', null, 'e', 'sw', 's', 'se'];
	for (var i = 0; i <= 8; i++) {
		if (i % 3 == 0) tr = hs.createElement('tr', null, { height: 'auto' }, tbody, true);
		this.td[i] = hs.createElement('td', null, null, tr, true);
		var style = i != 4 ? { lineHeight: 0, fontSize: 0, cursor: resizes[i] +'-resize'} : { position : 'relative' };
		hs.setStyles(this.td[i], style);
		this.td[i].className = 'highslide-resize';
	}
...
hs.Expander = function(a, params, custom, contentType) {
	...
	if (x.t == 0 && x.tpos == 0) { /// opening from an invisible anchor
	...
	/*<? endif ?>*/
		...
		if (p.min + p.span/*<? if ($cfg->overlays) : ?>s*/ + p.p1 + p.p2
			...
			/// simply move the slide to the right to align with the right of the viewport
				...
				/*<? endif ?>s*/ < p.clientSpan - p.marginMin - p.marginMax) {
				p.min = p.scroll + p.clientSpan - p.span - p.marginMax
					/*<? if ($cfg->overlays) : ?>s*/ - p.p1 - p.p2/*<? endif ?>*/;
...
reflow : function () {
	...
	this.content.style.height = this.y.span +'px';
	/*<? if ($cfg->outlines) : ?>s*/
...
show : function () {
	...
	var imgPos = {
		x: this.x.min - 20, 
		y: this.y.min - 20, 
		w: this.x.span + 40/*<? if ($cfg->overlays): ?>*/ + this.x.p1 + this.x.p2/*<? endif ?>*/, 
		h: this.y.span + 40/*<? if ($cfg->overlays): ?>*/ + this.y.p1 + this.y.p2/*<? endif ?>*/
	};
	hs.hideSelects = true;//(hs.ie && hs.ieVersion() < 7);
			...
			(function() {
				...
				setTimeout(function() {
					exp.outline.setPosition(exp, {
						x: (exp.x.min + o), 
						y: (exp.y.min + o),
						w: (exp.x.span - 2 * o/*<? if ($cfg->overlays) : ?>*/ + exp.x.p1 + exp.x.p2/*<? endif ?>*/), 
						h: (exp.y.span - 2 * o/*<? if ($cfg->overlays) : ?>*/ + exp.y.p1 + exp.y.p2/*<? endif ?>*/)
					}, 1);
				}, t);
			})();
		}
...
resize : function (e) {
	var w, h, l, t, r = e.width / e.height, dir = e.el.style.cursor.replace('-resize', '');
	...
	if (/n/.test(dir)) this.y.min = e.top + e.dY;
	this.x.span = w;
	this.y.span = h;
	/*<? if ($cfg->html) : ?>s*/
	if (this.isHtml) {
...
redoShowHide : function() {
	var imgPos = {
		...
		w: this.content.offsetWidth + 40/*<? if ($cfg->overlays): ?>*/ + this.x.p1 + this.x.p2/*<? endif ?>*/, 
		h: this.content.offsetHeight + 40/*<? if ($cfg->overlays): ?>*/ + this.y.p1 + this.y.p2/*<? endif ?>*/
	};
	if (hs.hideSelects) this.showHideElements('SELECT', 'hidden', imgPos);
	if (hs.hideIframes) this.showHideElements('IFRAME', 'hidden', imgPos);
	if (hs.geckoMac) this.showHideElements('*', 'hidden', imgPos);

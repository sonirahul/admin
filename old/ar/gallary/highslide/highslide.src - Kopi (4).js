...
updateAnchors : function() {
	var els = document.all || document.getElementsByTagName('*'), all = [], images = [], 
...
calcExpanded: function() {
	...
	this.minSize = /*<? if ($cfg->autosize) : ?>*/this.exp.allowSizeReduction ? 
		Math.min(this.exp['min'+ this.ucwh], this.full) :/*<? endif ?>*/ this.full;
	/*<? if ($cfg->overlays) : ?>s*/
	if (hs['padToMin'+ this.ucwh]) this.minSize = this.exp.minWidth;
	/*<? endif ?>s*/
	/*<? if ($cfg->positioning) : ?>s*/
...
htmlSizeOperations : function () {
    ...
    if (node 
    	...
    	var hDiff = /*<? if ($cfg->overlays) : ?>*/this.y.p1 + this.y.p2 + 
			/*<? endif ?>s*/this.innerContent.offsetHeight - node.offsetHeight;
...
correctRatio : function(ratio) {
	...
	if (/*<? if ($cfg->overlays) : ?>s*/!hs.padToMinWidth && 
			/*<? endif ?>s*/x.size / y.size > ratio) { // width greater
		x.size = y.size * ratio;
	...
	} else if (/*<? if ($cfg->overlays) : ?>s*/!hs.padToMinHeight && 
			/*<? endif ?>s*/x.size / y.size < ratio) { // height greater
		y.size = x.size / ratio;
		changed = true;
	}
	/*<? if ($cfg->overlays) : ?>s*/
...
setSize : function (to) {
	try {
		...
		hs.setStyles(this.content, {
			...
			/*<? endif ?>s*/
			width: (/*<? if ($cfg->overlays) : ?>s*/to.ximgSize || /*<? endif ?>s*/to.xsize) +'px',
			height: (/*<? if ($cfg->overlays) : ?>s*/to.yimgSize || /*<? endif ?>s*/to.ysize) +'px'
		});

...
justify : function (p) {
	...
	/*<? endif ?>*/
		...
		/*<? endif ?>*/
		// calculate p.min
		/*<? if ($cfg->positioning) : ?>*/if (p.justify == 'center') p.min = Math.round(p.scroll + (p.clientSpan - p.span - p.marginMax) / 2);
		else /*<? endif ?>*/p.min = Math.round(p.min - ((p.span - p.thumbSpan) / 2)); // auto
...
changeSize : function(up, from, to, dur, steps) {
	...
	if (!up /*<? if ($cfg->html) : ?>s*/ && !(this.isHtml && this.preserveContent)/*<? endif ?>s*/) { // remove children

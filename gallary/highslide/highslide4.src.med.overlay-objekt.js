/******************************************************************************
Name:    Highslide JS
Version: 4.0b3 (forked from 3.3.14)
Config: 
...
/*<? if ($cfg->html) : ?>s*/
// Navigation
/*<? if ($cfg->ajax || $cfg->iframe || $cfg->flash || $cfg->gallery) : ?>*/
...
skin : {
	...
	controls:
		'<div class="highslide-controls"><ul>'+
			'<li class="highslide-previous">'+
				'<a href="#" title="{hs.previousTitle}">'+
				'<span>{hs.previousText}</span></a>'+
			'</li>'+
			'<li class="highslide-play">'+
				'<a href="#" title="{hs.playTitle}">'+
				'<span>{hs.playText}</span></a>'+
			'</li>'+
			'<li class="highslide-pause">'+
				'<a href="#" title="{hs.pauseTitle}">'+
				'<span>{hs.pauseText}</span></a>'+
			'</li>'+
			'<li class="highslide-next">'+
				'<a href="#" title="{hs.nextTitle}">'+
				'<span>{hs.nextText}</span></a>'+
			'</li>'+
			'<li class="highslide-move">'+
				'<a href="#" title="{hs.moveTitle}">'+
				'<span>{hs.moveText}</span></a>'+
			'</li>'+
			'<li class="highslide-full-expand">'+
				'<a href="#" title="{hs.fullExpandTitle}">'+
				'<span>{hs.fullExpandText}</span></a>'+
			'</li>'+
			'<li class="highslide-close">'+
				'<a href="#" title="{hs.closeTitle}" >'+
				'<span>{hs.closeText}</span></a>'+
			'</li>'+
		'</ul></div>',
	/*<? endif ?>s*/
	/*<? if ($cfg->html) : ?>s*/
	contentWrapper:
		'<div class="highslide-header"><ul>'+
			'<li class="highslide-previous">'+
				'<a href="#" title="{hs.previousTitle}" onclick="return hs.previous(this)">'+
				'<span>{hs.previousText}</span></a>'+
			'</li>'+
			'<li class="highslide-next">'+
				'<a href="#" title="{hs.nextTitle}" onclick="return hs.next(this)">'+
				'<span>{hs.nextText}</span></a>'+
			'</li>'+
			'<li class="highslide-move">'+
				'<a href="#" title="{hs.moveTitle}" onclick="return false">'+
				'<span>{hs.moveText}</span></a>'+
			'</li>'+
			'<li class="highslide-close">'+
				'<a href="#" title="{hs.closeTitle}" onclick="return hs.close(this)">'+
				'<span>{hs.closeText}</span></a>'+
			'</li>'+
		'</ul></div>'+
...
/*<? endif // html ?>s*/
/*<? if ($cfg->ajax || $cfg->iframe || $cfg->flash || $cfg->gallery) : ?>s*/
replaceLang : function(s) {
	...
	for (var i = 0; i < matches.length; i++) {
		...
		if (hs[lang]) s = s.replace(matches[i], hs[lang]);
	}
	return s;
},
/*<? endif ?>*/
...
//-----------------------------------------------------------------------------
// The overlays object
hs.Overlays = function(exp) {
	try {
	this.exp = exp;
	this.overlays = [];
	this.x = exp.x;
	this.y = exp.y;
	this.getCaptionAndHeader();
	if (hs.showCredits) this.writeCredits();
	for (var i = 0; i < hs.overlays.length; i++) {
		var o = hs.overlays[i], tId = o.thumbnailId, sg = o.slideshowGroup;
		if ((!tId && !sg) || tId == exp.thumbsUserSetId
				|| sg === exp.slideshowGroup) {
			/*<? if ($cfg->html) : ?>s*/
			if (exp.isImage || (exp.isHtml && o.useOnHtml))
			/*<? endif ?>s*/
	...
	/// set appropriate right and left
	if (this.overlayBox) this.sizeOverlayBox(0, 1);
	return this;
	} catch (e) { console.error( e.lineNumber, e.message) }
};
hs.Overlays.prototype = {
getCaptionAndHeader : function() {
	var exp = this.exp;
	/*<? if ($cfg->events) : ?>s*/
	if (!hs.fireEvent(exp, 'onBeforeGetCaption')) return;
	/*<? endif ?>s*/
	for (var i = 0; i <= 1; i++) {
		var type = i ? 'caption' : 'header', s; /// two types of caption: caption and header
		/// 1. The old caption-for-thumb syntax
		if (!exp[type +'Id'] && exp.thumbsUserSetId)  
			exp[type +'Id'] = type +'-for-'+ exp.thumbsUserSetId;
		/// 2. The captionId syntax + #1
		if (exp[type +'Id']) exp[type] = hs.getNode(exp[type +'Id']);
		/// 3. The captionEval syntax
		if (!exp[type] && !exp[type +'Text'] && exp[type +'Eval']) try {
			s = eval(exp[type +'Eval']);
		} catch (e) {}
		/// 4. The captionText syntax
		if (!exp[type] && exp[type +'Text']) {
			s = exp[type +'Text'];
		}
		/// 5. Look for subsequent caption div
		if (!exp[type] && !s) {
			var next = exp.a.nextSibling;
			while (next && !hs.isHsAnchor(next)) {
				if ((new RegExp('highslide-'+ type)).test(next.className || null)) {
					exp[type] = next.cloneNode(1);
					break;
				}
		...
		/*<? if ($cfg->gallery) : ?>s*/
		/// 6. Look for numbers
		if (!exp[type] && !s && exp.number == type) s = '';
		/*<? endif ?>s*/			
		
		if (!exp[type] && typeof s != 'undefined') exp[type] = hs.createElement('div', 
				{ className: 'highslide-'+ type, innerHTML: s } );
				
		if (exp[type]) {
			/*<? if ($cfg->dragging) : ?>s*/
			if (type == 'header' && hs.dragHeader) exp[type].className += ' highslide-move';
			/*<? endif ?>s*/
			
			/* this.createOverlay({
				overlayId: this[type],
				position: this[type+'Position']
			}); */
			
			
			var o = {}; /// defaults for header
			for (var x in exp[type+'Options']) o[x] = exp[type+'Options'][x];
			o.overlayId = exp[type];
			this.createOverlay(o);
		}
	}
	/*<? if ($cfg->gallery) : ?>s*/
	...
	/*<? endif ?>s*/
	/*<? if ($cfg->events) : ?>s*/
	if (exp.caption) hs.fireEvent(exp, 'onAfterGetCaption');
	/*<? endif ?>s*/
},
writeCredits : function () {
	/*<? if ($cfg->gallery) : ?>s*/
	if (this.exp.credits) return;
	/*<? endif ?>s*/
	this.exp.credits = hs.createElement('a',
		{
			href: hs.creditsHref,
...
createOverlay : function (o) {
	...
	/*<? if ($cfg->events) : ?>s*/
	if (!hs.fireEvent(this.exp, 'onCreateOverlay', { overlay: el })) return;
	/*<? endif ?>s*/
	el.style.display = 'block';
	/*<? if ($cfg->transitions) : ?>s*/
	o.hsId = o.hsId || o.overlayId; 
	if (this.exp.transitions[1] == 'crossfade' && this.reuseOverlay(o, el)) return;
	/*<? endif ?>s*/
...
positionOverlay : function(overlay, conH) {
	...
	/*for (var dim in hs.oPos) {
		var hor = (dim == 'x'),
			lt = hor ? 'left' : 'top',
			mlt = hor ? 'marginLeft' : 'marginTop',
			rb = hor ? 'right' : 'bottom';
		
		if (p.match(hs.oPos[dim][0])) { /// leftpanel/above
			
		
		} else if (p.match(hs.oPos[dim][1])) { /// left/top
			overlay.style[lt] = 0; 
		
		} else if (p.match(hs.oPos[dim][2])) { /// center/middle
			overlay.style[lt] = '50%';
			overlay.style[mlt] = '-'+ Math.round(overlay.offsetWidth / 2) +'px';
		
		} else if (p.match(hs.oPos[dim][3])) { /// right/bottom
			overlay.style[rb] = 0;
		}
...
sizeOverlayBox : function(doWrapper, doPanels) {
	hs.setStyles( this.overlayBox, {
		width: (this.padded ? this.x.minSpan : this.x.span) +'px', 
...
createFullExpand : function () {
	/*<? if ($cfg->gallery) : ?>s*/
	if (this.exp.slideshow) {
		this.exp.slideshow.enable('full-expand');
		return;
	}
	/*<? endif ?>s*/
	this.fullExpandLabel = hs.createElement(
		'a',
		{
			href: 'javascript:hs.expanders['+ this.exp.key +'].doFullExpand();',
	...
	/*<? if ($cfg->events) : ?>s*/
	if (!hs.fireEvent(this.exp, 'onCreateFullExpand')) return;
	/*<? endif ?>s*/
	
	this.createOverlay({ 
		overlayId: this.exp.fullExpandLabel, 
...
showOverlays : function() {
	...
	hs.setStyles(this.overlayBox, {
		top: (this.y.p1 + this.y.cb) +'px',
		left: (this.x.p1 + this.x.cb + (this.padded ? this.x.cb + this.x.span + this.x.p2 - this.x.minSpan : 0)) +'px',
		overflow : 'visible'
	});
	this.exp.wrapper.appendChild (this.overlayBox);
	
	if (hs.safari) { /// display bug causing overlays to keep hidden
		var stl = this.exp.content.style;
		stl.display = 'inline';
	...
	for (var i = 0; i < this.overlays.length; i++) {
		var o = hs.$('hsId'+ this.overlays[i]);
		if (!o.hideOnMouseOut || this.exp.mouseIsOver) hs.fade(o, 0, o.opacity);
	}
...
hs.Expander = function(a, params, custom, contentType) {
	...
	*/
	...
	// initiate metrics
	var x = this.x = {};
	x.t = el.width || el.offsetWidth; /// thumb width
	x.tpos = pos.x; /// thumb left
	x.tb = (el.offsetWidth - x.t) / 2; /// thumb border
	var y = this.y = {};
	y.t = el.height || el.offsetHeight;
	y.tpos = pos.y;
...
contentLoaded : function() {
	try {	
		...
		this.initSlideshow();
		/*<? endif ?>s*/
		this.overlays = new hs.Overlays(this);
		/*<? endif ?>s*/
		...
		/*<? endif ?>s*/
		/*<? if ($cfg->overlays) : ?>s*/
		if (this.overlays) this.overlays.sizeOverlayBox(0, 1);
		/*<? endif ?>s*/
		/*<? if ($cfg->autosize) : ?>*/
		/*<? if ($cfg->html) : ?>s*/
		if (this.isImage) 
...
reflow : function () {
	...
	/*<? if ($cfg->outlines) : ?>s*/
	if (this.objOutline) this.objOutline.setPosition(this);
	/*<? endif ?>s*/
},
...
changeSize : function(up, from, to, dur, steps) {
	...
	if (this.objOutline && !this.outlineWhileAnimating) {
		if (up) this.objOutline.setPosition(this);
...
setSize : function (to) {
	try {		
		...
		if (this.objOutline && this.outlineWhileAnimating) {
			...
			this.objOutline.setPosition(this, {
				x: to.xmin + o, 
				y: to.ymin + o, 
	...
	if (this.objOutline) {
		...
		for (var i = from.o; dir * i <= dir * to.o; i += dir, t += 25) {
			(function() {
				...
				setTimeout(function() {
					exp.objOutline.setPosition(exp, {
						x: (exp.x.min + o), 
						y: (exp.y.min + o),
						w: (exp.x.span - 2 * o), 
						h: (exp.y.span - 2 * o)
...
crossfade : function (up, from, to) {
	...
	/*<? if ($cfg->outlines) : ?>s*/
	this.objOutline = this.last.objOutline;
	this.last.objOutline = null;
	/*<? endif ?>s*/
	
	// dummy borders
	this.fadeBox = hs.createElement('div', 
		{ className: 'highslide-image' }, /// for borders
		{ 
			position: 'absolute', 
			zIndex: 4,
			overflow: 'hidden',
	...
	/*
	hs.setStyles(this.last.overlayBox, {
			opacity: 0.95
		});
	console.log(1 - from.o);
	return;
	*/
...
afterExpand : function() {
	...
	/*<? if ($cfg->overlays) : ?>*/
	if (this.overlays) this.overlays.showOverlays();
	/*<? endif ?>s*/
},
...
getNumber : function() {
	var arr = hs.anchors.groups[this.slideshowGroup || 'none'];
	var s = hs.skin.number.replace('%1', this.getAnchorIndex() + 1).replace('%2', arr.length);
	/// prepend to caption or header
	if (this[this.number]) this[this.number].innerHTML = s + this[this.number].innerHTML;
},
...
resize : function (e) {
	...
	this.x.span = Math.max(e.width + e.dX, this.minWidth);
	if (this.isImage && Math.abs(this.x.span - this.x.full) < 12) this.x.span = this.x.full; /// snap to actual size
	this.y.span = /*<? if ($cfg->html) : ?>s*/ this.isHtml ? e.height + e.dY : /*<? endif ?>s*/ this.x.span / r;
	
	if (this.y.span < this.minHeight) {
		this.y.span = this.minHeight;
		if (this.isImage) this.x.span = this.y.span * r;
	}
	...
	/*<? endif ?>s*/
	/*<? if ($cfg->overlays) : ?>s*/
	if (this.overlays) this.overlays.sizeOverlayBox(true);
	/*<? endif ?>s*/	
...
sleep : function() {
	/*<? if ($cfg->outlines) : ?>s*/
	if (this.objOutline) this.objOutline.table.style.display = 'none';
	/*<? endif ?>s*/
	/*<? if ($cfg->iframe) : ?>s*/
...
doFullExpand : function () {
	try {
		...
		this.wrapper.style.width = (this.x.span + 2 * this.x.cb) +'px';
...
loadHTML : function() {
	...
	if (!hs.ie || hs.ieVersion() >= 5.5) {
		s = s.replace(/\n/g, '_hsnl_').replace(/\r/g, ''); /// remove newlines

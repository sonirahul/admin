...
skin : {
	...
	controls:
		...
		'</ul></div>'
	/*<? endif ?>s*/
	/*<? if ($cfg->html && $cfg->slideshow) : ?>s*/,/*<? endif ?>s*/
	/*<? if ($cfg->html) : ?>s*/
	contentWrapper:
...
isUnobtrusiveAnchor: function(el) {
	...
	/*<? if ($cfg->ajax || $cfg->iframe || $cfg->flash) : ?>s*/
...
fixCursor: function() { /// Fix IE relative cursor path bug
...
setPosition : function (exp, pos, vis) {
	pos = pos || {
		x: exp.x.pos,
		y: exp.y.pos,
		w: exp.x.size/*<? if ($cfg->overlays) : ?>*/ + exp.x.p1 + exp.x.p2/*<? endif ?>*/,
		h: exp.y.size/*<? if ($cfg->overlays) : ?>*/ + exp.y.p1 + exp.y.p2/*<? endif ?>*/
	};
...
get : function(key) {
	switch (key) {
		...
		case 'opos': /// position of the outline
			return this.pos/*<? if ($cfg->outlines)?>s*/ - (this.exp.outline ? this.exp.outline.offset : 0);
		case 'osize': /// outer size of the outline
			return this.get('wsize')/*<? if ($cfg->outlines)?>s*/ + (this.exp.outline ? 2*this.exp.outline.offset : 0);
...
calcThumb: function() {
	...
	/*<? endif ?>s*/	
	if (this.t == 0 && this.tpos == 0) { /// opening from an invisible anchor
...
calcExpanded: function() {
	...
	this.minSize = /*<? if ($cfg->autosize) : ?>*/this.exp.allowSizeReduction 
		? this.exp['min'+ this.ucwh] : /*<? endif ?>*/this.full;
	/*<? if ($cfg->positioning) : ?>s*/
...
setSize: function(i) {
	...
	if (this.exp.isHtml) {
		...
		if (this.dim == 'x') this.exp.mediumContent.style.width = 'auto'; /// why not do it after expand?
...
hs.Expander = function(a, params, custom, contentType) {
	...
	var x = this.x = new hs.Metrics(this, 'x');
	x.calcThumb();
	var y = this.y = new hs.Metrics(this, 'y');
	y.calcThumb();
...
setSize : function (to) {
	try {		
		...
		hs.setStyles ( this.wrapper, {
			width : (to.xsize + /*<? if ($cfg->overlays) : ?>s*/to.xp1 + to.xp2 + /*<? endif ?>s*/
				2 * this.x.cb) +'px',
			height : (to.ysize + /*<? if ($cfg->overlays) : ?>s*/to.yp1 + to.yp2 + /*<? endif ?>s*/
				2 * this.y.cb) +'px',
			left: to.xpos +'px',
			top: to.ypos +'px'
		});
		...
		if (this.isHtml) {
			hs.setStyles(this.mediumContent, { 
				left: (this.x.pos - to.xpos/*<? if ($cfg->overlays) : ?>s*/ 
					+ this.x.p1 - to.xp1/*<? endif ?>s*/) +'px',
				top: (this.y.pos - to.ypos/*<? if ($cfg->overlays) : ?>s*/ 
		...
		if (this.outline && this.outlineWhileAnimating) {
			...
			this.outline.setPosition(this, {
				x: to.xpos + o, 
				y: to.ypos + o, 
				w: to.xsize /*<? if ($cfg->overlays) : ?>s*/ + to.xp1 + to.xp2 + /*<? endif ?>s*/ - 2 * o, 
				h: to.ysize /*<? if ($cfg->overlays) : ?>s*/ + to.yp1 + to.yp2 + /*<? endif ?>s*/ - 2 * o
			}, 1);
		}
		/*<? endif ?>*/	
			...
			(function() {
				...
				setTimeout(function() {
					exp.outline.setPosition(exp, {
						x: (exp.x.pos + o), 
						y: (exp.y.pos + o),
						w: (exp.x.size - 2 * o/*<? if ($cfg->overlays) : ?>*/ + exp.x.p1 + exp.x.p2/*<? endif ?>*/), 
						h: (exp.y.size - 2 * o/*<? if ($cfg->overlays) : ?>*/ + exp.y.p1 + exp.y.p2/*<? endif ?>*/)
					}, 1);
				}, t);
			})();
		}
...
crossfade : function (up, from, to) {
	...
	if (steps > 1) this.crossfadeStep(from); /// initial position	
...
crossfadeStep : function (size) {
	try {
		...
		hs.setStyles(this.oldImg, {
			...
			left: (this.last.x.pos - size.xpos + this.last.x.p1 - size.xp1)+'px'
		});		
		
		hs.setStyles(this.newImg, {
			opacity: size.o,
			top: (this.y.pos - size.ypos + this.y.p1 - size.yp1) +'px',
			left: (this.x.pos - size.xpos + this.x.p1 - size.xp1) +'px'
		});
		/*<? if ($cfg->overlays) : ?>s*/
...
genOverlayBox : function() {
	if (!this.overlayBox) this.overlayBox = hs.createElement (
		'div', {
			...
			position : 'absolute',
			width: this.x.size ? this.x.size +'px' : this.x.full +'px', /// initial overlay size
			height: 0,
			visibility : 'hidden',

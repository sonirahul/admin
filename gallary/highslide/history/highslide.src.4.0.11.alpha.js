		...
		if (p.pos + p.get('wsize') > p.scroll + p.clientSize - p.marginMax) {
			...
			if (!moveOnly && hasMovedMin/*<? if ($cfg->autosize) : ?>*/ && allowReduce/*<? endif ?>*/) {
				...
				p.size = p.get('fitsize')
					/*<? if ($cfg->overlays) : ?>s*/- 2 * p.cb - p.p1 - p.p2/*<? endif ?>s*/; // can't expand more
				/*<? endif ?>s*/
			...
			/// c) if the slide is still larger than the viewport, align it left and fit the width
				...
				if (!moveOnly && allowReduce) p.size = p.get('fitsize')
					/*<? if ($cfg->overlays) : ?>s*/- 2 * p.cb - p.p1 - p.p2/*<? endif ?>s*/;
				/*<? endif ?>s*/
			}			
		}
...
setSize : function (to) {
	try {
		...
		hs.setStyles(this.content, {
			...
			/*<? endif ?>s*/
			width: (/*<? if ($cfg->autosize) : ?>s*/to.ximgSize || /*<? endif ?>s*/to.xsize) +'px',
			height: (/*<? if ($cfg->autosize) : ?>s*/to.yimgSize || /*<? endif ?>s*/to.ysize) +'px'
		});
